...
int main() {
  ...
  rep(i,m){
    int ai = inp[i].first;
    int bi = inp[i].second;
    if(tree.same(ai,bi)) {answers.push_back(sum); continue;}
    ll anum = mp[tree.root(ai)];
    ll bnum = mp[tree.root(bi)];
    tree.unite(ai,bi);
    ll newnum = anum + bnum;
    mp[tree.root(ai)] = newnum;
    sum = sum + (anum * (anum-1) / 2) + (bnum * (bnum-1) / 2) - (newnum * (newnum-1) / 2);
    answers.push_back(sum);

  }
  reverse(all(answers));
  rep(i,answers.size()-1){
