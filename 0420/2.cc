...
int main() {
  ...
  rep(i,n){
    int si, ti,ci;
    cin >> si >> ti >> ci;
    v.emplace_back(si,ti,ci);
  }

  sort(all(v));

  vector<pair<int,int>> times(C+35);
  rep(i,C+35) {times[i].first = INF;times[i].second = INF;}
  int ans = 0;
  ...
  rep(i,v.size()){
    ...
    sort(all(times));
    rep(j,ans){
      if(times[j].second == ci){
        if(times[j].first <= si) {times[j].first = ti; flag = 1; break;}
      }
      if(times[j].first < si) {times[j].first = ti; times[j].second = ci; flag = 1; break;}
    }
    if(flag) continue;
    //もし空いている録画機がなかったら
    times[ans].first = ti;
