...
int main() {
  ...
  cin >> n >> k;
  vector<ll> A(n);
  rep(i,n) {int ai; cin >> ai; A[i] = ai-1;}
  ll ans = 0;
  ...
  while(1){
    ...
    if(mp[next] == 0){
      ...
      //loopによって求めざるを得ない時のお話
      //count 番目がnext
      ll routine = count - mp[next];
      cout << count << " " << mp[next] << endl;
      ll modr = (k - (mp[next]-1)) % routine;
      cout << modr << endl;
      //ループの中でmodr番目(0-index)に訪れる箇所を見る
      ll find = mp[next] + modr;
      cout << find << endl;
      ll sta = 0;
      for(ll j = 0; j < find-1; j++){
        sta = A[sta];
      }
      ans = sta+1;
      break;
