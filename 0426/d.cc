...
int main() {
  ...
  rep(i,n){
    int digit = s[i] - '0';
    temp63 = (temp63 + digit * ten63) % 673;
    temp3 = (temp3 + digit * ten3) % 3;
    ten63 = (ten63 * 10) % 673;
    ten3 = (ten3 * 10) % 3;
    v.push_back(pair<int,int>(temp63,temp3));
  }
  ...
  ll ans = 0;
  rep(i,v.size()){
    if(mp[v[i]] == 0) mp[v[i]] = mp[v[i]] + 1;
    else {mp[v[i]] = mp[v[i]] + 1; ans = ans + (mp[v[i]] * (mp[v[i]]-1))/2 - ((mp[v[i]]-1) * (mp[v[i]]-2))/2;}
  }
